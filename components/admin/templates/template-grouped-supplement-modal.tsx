/**
 * Template Grouped Supplement Modal
 *
 * Modal for viewing and managing multiple supplements in a grouped time slot within a template.
 * Adapted from GroupedSupplementModal for template items.
 */
 ...
 */
function formatSupplementDetails(item: TemplateSupplementItemWithSupplement): string {
  const dosage = item.dosage || 1;
  const unit = item.supplements?.dosage_unit || "unit";
  return `${dosage} ${unit}`;
}

/**
 * Modal for managing grouped template supplement items
 */
export function TemplateGroupedSupplementModal({
  isOpen,
  onClose,
  onAddItem,
  onEditItem,
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {groupedItems.map((templateItem, index) => {
              const supplement = templateItem.supplements;
              const name = supplement?.name || "Unknown Supplement";
              const details = formatSupplementDetails(templateItem);
